...
- Correct flow per slide: write → save(measure=[slug]) → preview → fix if needed → next slug.
...
Do NOT call `get_preview` in this mode. Fix via `run_python(save=True, measure_slides=[...])`.
